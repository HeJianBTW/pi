...
// Mock the provider creation so we don't need real Mem0 infra
...
function mockProviderWith(items: MemoryItem[]) {
  ...
  const flushSnapshotMock = vi.fn().mockResolvedValue(undefined);
  mockCreateProvider.mockResolvedValue({
    add: vi.fn(),
    search: vi.fn(),
    getAll: vi.fn().mockResolvedValue(items),
    delete: deleteMock,
    flushSnapshot: flushSnapshotMock,
  });
  return { deleteMock, flushSnapshotMock };
}
...
describe('dedupMemories', () => {
  ...
  it('returns zero duplicates when all entries are unique', async () => {
    const { deleteMock, flushSnapshotMock } = mockProviderWith([
  ...
  it('deletes exact duplicates (case-insensitive), keeps newest', async () => {
    ...
    expect(deleteMock).toHaveBeenCalledWith('1'); // older one deleted
    expect(flushSnapshotMock).toHaveBeenCalledWith('test-user');
  });

  it('deletes duplicates with whitespace normalization', async () => {
  ...
  it('respects abort signal during scan', async () => {
