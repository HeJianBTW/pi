...
import type { ConversationTurn } from "@amaster.ai/pi-types";
import { JsonFileConversationStore, JsonFileTranscriptStore } from "./session-stores.js";
import type { RuntimeSession } from "@amaster.ai/pi-types";
...
describe("JsonFileTranscriptStore", () => {
  ...
  it("returns session summaries from persisted summary state", async () => {
    ...
    expect(await store.listSessionSummaries({ tenantId: "tenant-1" }, [
      {
        sessionId: "session-1",
  ...
  it("treats legacy JSON sessions without tenantId as default tenant sessions", async () => {
    ...
    await expect(store.getRuntimeSession({ tenantId: "default" }, "legacy-session")).resolves.toMatchObject({
      sessionId: "legacy-session",
    });
...
function turn(id: string, sessionId: string, userMessage: string, assistantMessage: string): ConversationTurn {
  return {
    id,
    sessionId,
