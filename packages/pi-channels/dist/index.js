...
export default function piChannelsExtension(pi) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            payloadMode: Type.Optional(enumSchema(['envelope', 'raw'], 'Webhook payload mode')),
        ...
        async execute(_toolCallId, rawParams) {
            ...
            if (params.json) {
                try {
                    ...
                    return textToolResult(`Invalid JSON: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            const webhook = {};
            if (params.method)
    ...
    pi.events.on('channel:remove', (raw) => {
        const data = raw;
        data.callback?.(data.name ? registry.unregister(data.name) : false);
    });
    pi.events.on('channel:list', (raw) => {
        const data = raw;
...
function autoFillEmptyRouteRecipient(cwd, projectTrusted, message, log) {
