export function createWebhookAdapter(config) {
    ...
    const defaultContentType = typeof config.contentType === 'string' ? config.contentType : 'application/json';
    const defaultPayloadMode = config.payloadMode === 'raw' ? 'raw' : 'envelope';
    ...
    if (secret && !hasAuthorization)
        headers.Authorization = `Bearer ${secret}`;
    return {
        direction: 'outgoing',
        async send(message) {
            const payloadMode = message.payloadMode ?? defaultPayloadMode;
            const method = payloadMode === 'raw' ? (message.webhook?.method ?? defaultMethod) : defaultMethod;
