...
describe("subagents", () => {
  ...
  it("resolves named roles case-insensitively", () => {
    ...
    expect(resolveSubagentRole(registry, "missing")).toBe("subagent role not found: missing. Available roles: reviewer");
  });
  ...
  it("spawns a child run through injected orchestration and inherits parent request semantics", async () => {
    ...
    const result = await spawnSubagentRun({
      ...
      logger: noopLogger,
      metrics: { subagentsSpawnedTotal: 0, subagentTurnsCompletedTotal: 0, subagentTurnsFailedTotal: 0, subagentsCancelledTotal: 0 },
      cancelChildSubagentsForParent: async () => 0,
      promptSubagentTurn: async (active) => {
        (active.pi.messages as unknown[]).push({ role: "assistant", content: [{ type: "text", text: "contract looks good" }] });
      },
...
class MemorySubagentRunStore implements SubagentRunStore {
  ...
  async list(_scope: Parameters<SubagentRunStore["list"]>[0], parentSessionId?: string): Promise<SubagentRun[]> {
    return [...this.runs.values()].filter((run) => !parentSessionId || run.parentSessionId === parentSessionId);
  }

  async get(_scope: Parameters<SubagentRunStore["get"]>[0], runId: string): Promise<SubagentRun | undefined> {
  ...
  async markRunning(_scope: Parameters<SubagentRunStore["markRunning"]>[0], runId: string): Promise<SubagentRun | undefined> {
  ...
  async markCompleted(_scope: Parameters<SubagentRunStore["markCompleted"]>[0], runId: string, result: string): Promise<SubagentRun | undefined> {
    return this.update(runId, {
      status: "completed",
      result,
  ...
  async markFailed(_scope: Parameters<SubagentRunStore["markFailed"]>[0], runId: string, error: string): Promise<SubagentRun | undefined> {
    return this.update(runId, {
      status: "failed",
      error,
      endedAt: "2026-05-15T00:00:02.000Z",
  ...
  async markCancelled(_scope: Parameters<SubagentRunStore["markCancelled"]>[0], runId: string, reason?: string): Promise<SubagentRun | undefined> {
    return this.update(runId, {
      status: "cancelled",
      ...(reason ? { error: reason } : {}),
      endedAt: "2026-05-15T00:00:02.000Z",
  ...
  private update(runId: string, patch: Partial<SubagentRun> & { events?: SubagentRun["events"] }): SubagentRun | undefined {
