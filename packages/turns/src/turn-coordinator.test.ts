...
describe('TurnCoordinator', () => {
  it('rejects overlapping turns for the same session by default', async () => {
    ...
    const first = coordinator.run(
      { tenantId: 'tenant-1', sessionId: 'session-1', source: 'chat' },
      async () => {
  ...
  it('queues same-session turns when requested', async () => {
    ...
    expect(
      coordinator.getSessionStatus({ tenantId: 'tenant-1', sessionId: 'session-1' }),
    ).toMatchObject({
      active: true,
      pendingCount: 2,
      queuedCount: 1,
  ...
  it('tracks main and subagent concurrency separately', async () => {
    ...
    const main = coordinator.run(
      { tenantId: 'tenant-1', sessionId: 'main-1', source: 'chat' },
      async () => {
        order.push('main-1');
        mainStarted.resolve();
    ...
    const subagent = coordinator.run(
      { tenantId: 'tenant-1', sessionId: 'subagent-1', source: 'subagent' },
      async () => {
    ...
    const queuedMain = coordinator.run(
      { tenantId: 'tenant-1', sessionId: 'main-2', source: 'chat' },
      async () => {
        order.push('main-2');
        return 'main-2';
      },
    );
    const queuedSubagent = coordinator.run(
      { tenantId: 'tenant-1', sessionId: 'subagent-2', source: 'subagent' },
      async () => {
...
function createCoordinator(
  options: Partial<ConstructorParameters<typeof TurnCoordinator>[0]> = {},
): TurnCoordinator {
