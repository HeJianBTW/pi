...
import type { RuntimeLifecycleEvent, RuntimeToolEvent } from "@amaster.ai/pi-types";
import {
...
describe("telemetry", () => {
  ...
  it("flushes Langfuse batches with basic auth", async () => {
    const requests: Array<{ input: string; init: { headers: Record<string, string>; body: string } }> = [];
  ...
  it("nests subagent model and tool observations under the subagent span", async () => {
    ...
    const spawnToolSpan = rootSpan?.spans.find((span) => span.body.name === "sessions_spawn [child task]");
...
function getStringAttribute(span: { attributes: Array<{ key: string; value: { stringValue?: string } }> }, key: string): string | undefined {
