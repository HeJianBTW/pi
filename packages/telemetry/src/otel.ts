...
export { OtelRuntimeEventExporter, type OtelExporterConfig, type TelemetryFetch };
...
export function createOtelRuntimeEventExporterFromEnv(
  ...
  return config.enabled ? new OtelRuntimeEventExporter(config, fetchImpl) : new NoopRuntimeEventExporter();
}
